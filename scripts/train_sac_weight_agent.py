...
from research.training.sac_trainer import DEFAULT_REWARD_VERSION, EvaluationResult, evaluate, train_sac
...
SAC_POLICY_PRESETS: Dict[str, Dict[str, Any]] = {
    "sac_core_v1": {
        "policy": "MlpPolicy",
        "timesteps": 10_000,
        "learning_rate": 3e-4,
...
DEFAULT_SAC_PRESET = "sac_core_v1"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a SAC agent on canonical equity features.")
    ...
    parser.add_argument("--seed", type=int, default=42, help="Random seed passed to SAC.")
    parser.add_argument(
        "--policy-preset",
        default=DEFAULT_SAC_PRESET,
        help="SAC policy preset name (hyperparameters resolved from the spec).",
    )
    parser.add_argument(
        "--reward-version",
...
def _resolve_policy_config(args: argparse.Namespace) -> Dict[str, Any]:
    policy_params = getattr(args, "policy_params", {}) or {}
    if not isinstance(policy_params, Mapping):
        raise SystemExit("policy_params must be a mapping for SAC training.")
    preset = getattr(args, "policy_preset", None) or getattr(args, "policy", None) or DEFAULT_SAC_PRESET
    preset_key = str(preset).strip().lower()
    if preset_key == "sac":
        preset_key = DEFAULT_SAC_PRESET
    base = SAC_POLICY_PRESETS.get(preset_key)
    if base is None:
        available = ", ".join(sorted(SAC_POLICY_PRESETS))
        raise SystemExit(f"Unknown SAC policy preset '{preset}'. Available presets: {available}")
    merged = dict(base)
    for key, value in policy_params.items():
        merged[key] = value
    merged["policy_preset"] = preset_key
    return merged
...
def run_training(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if interval != "daily":
        raise SystemExit("Only the daily interval is supported for SAC training in v1.")
    ...
    policy_config = _resolve_policy_config(args)
    reward_version = policy_config.get("reward_version") or getattr(args, "reward_version", DEFAULT_REWARD_VERSION)
    reward_version = _validate_reward_version(reward_version)
    ...
    fast_window = int(policy_config.get("fast_window", args.fast_window))
    slow_window = int(policy_config.get("slow_window", args.slow_window))
    if fast_window <= 0 or slow_window <= 0:
        raise SystemExit("SMA windows must be positive.")
    if fast_window >= slow_window:
        raise SystemExit("fast-window must be strictly less than slow-window.")
    ...
    if not per_symbol_features:
        raise SystemExit("Failed to construct feature frames for SAC training.")
    ...
    if len(rows) < 2:
        raise SystemExit("Not enough feature rows to train SAC.")
    ...
    timesteps = _require_int(policy_config, "timesteps")
    policy_id = str(policy_config.get("policy") or "MlpPolicy")
    ...
    if policy_kwargs is not None and not isinstance(policy_kwargs, Mapping):
        raise SystemExit("policy_kwargs must be a mapping when provided.")
    sac_kwargs = _build_sac_kwargs(policy_config)
    ...
    train_report_path = reports_dir / f"sac_train_{run_id}.json"
    eval_report_path = reports_dir / f"sac_eval_{run_id}.json"
    plot_path = plots_dir / f"sac_eval_{run_id}.png"
    ...
    model_path = save_model(model, data_root / "models" / f"sac_{run_id}.zip")
...
def _weights_series_for_report(
    ...
    return {symbol: [entry.get(symbol, 0.0) for entry in weight_entries] for symbol in symbol_order}
...
def build_train_report(
    *,
    ...
    training_config = {
        ...
        "policy_preset": (policy_config or {}).get("policy_preset"),
        "duration_seconds": duration_seconds,
    }
    if policy_config:
        training_config["sac_kwargs"] = {
...
def build_eval_report(
    *,
    ...
    payload = {
        ...
        "interval": interval,
        "date_range": {"start": start.isoformat(), "end": end.isoformat()},
        "performance": eval_result.metrics,
        "env_config": {
            "initial_cash": env_config.initial_cash,
            "transaction_cost_bp": env_config.transaction_cost_bp,
            "risk_config": env_config.risk_config.to_dict(),
...
def derive_run_id(
    ...
    digest = hashlib.sha256(
        json.dumps(base_payload, sort_keys=True, separators=(",", ":"), ensure_ascii=False).encode("utf-8")
    ).hexdigest()
    return f"sac_weight_{digest[:12]}"
...
def _build_sac_kwargs(policy_config: Mapping[str, Any]) -> Dict[str, Any]:
    kwargs: Dict[str, Any] = {}
    for key in (
        "buffer_size",
        "batch_size",
...
def _require_int(params: Mapping[str, Any], key: str) -> int:
    if key not in params:
        raise SystemExit(f"{key} must be provided in SAC policy config.")
    try:
        return int(params[key])
    except (TypeError, ValueError) as exc:  # pragma: no cover
        raise SystemExit(f"{key} must be an integer for SAC training.") from exc
